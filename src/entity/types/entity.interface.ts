 ...
 * validation. Symbols (rather than string keys) keep these properties out of
 * any iteration over the entity's string keys, so they never leak into a
 * serialised payload.
 *
...
export interface IEntity<SchemaType extends t.TSchema> extends IRawEntity {
	...
	/**
	 * Builds a fresh `IValueObjectContext` payload tagged with `[EntitySource]`.
	 ...
	 */
	[EntityContext](name: string): IValueObjectContext;
}
