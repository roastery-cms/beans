import { BooleanVO, StringVO, SlugVO } from "@/collections/value-objects";
import { Entity } from "@/entity";
import { blueprint } from "@/entity/helpers";
import type {
	AccessorsOf,
	EntityDefinition,
	RulesOf,
...
/** Vocabulário de domínio: aliases que herdam `defineMeta` e `transform`. */
...
/** Derivação que lê um irmão preenchido por `default`. */
...
/** Derivações encadeadas, na ordem do blueprint. */
...
/** Blueprint sem regras — o caminho que existia antes. */
...
describe("blueprint", () => {
	...
	describe("construction", () => {
		...
		it("derives from a sibling filled by a default", () => {
			const banner = new Banner({ title: "Lançamento" });

			expect(banner.get("prefix")).toBe("tag");
			expect(banner.get("label")).toBe("tag: Lançamento");
		});
	...
	describe("nested entities", () => {
		...
		it("applies the nested entity's rules to its raw payload", () => {
			const post = new Post({ title: "Olá", tag: { name: "Alan Reis" } });
