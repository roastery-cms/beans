...
import { DefinedStringVO } from "@/collections/value-objects/new";
import {
	type AccessorsOf,
	BetterEntity,
...
/**
 * VO com `transform` — existe para cobrir o no-op *após* normalização em
 * `set()`: atribuir `"Hello World"` a um slug que já vale `"hello-world"` não
...
class Bean extends BetterEntity<typeof beanProperties> {
...
class Author extends BetterEntity<typeof authorProperties> {
...
class Tracked extends BetterEntity<typeof trackedProperties> {
...
describe("BetterEntity", () => {
	describe("defineEntity", () => {
		...
		it("satisfies the IBetterEntity contract for its own blueprint", () => {
			const bean: IBetterEntity<typeof beanProperties> = makeBean();
		...
		it("rejects an implementation declared as a class field", () => {
			class Broken extends BetterEntity<typeof authorProperties> {
				protected defineEntity = (): EntityDefinition<
					typeof authorProperties
				> => ({ properties: authorProperties, source: "broken" });
			}

			expect(() => Broken.demo()).toThrow(OperationFailedException);
		});
	});
	...
	describe("construction", () => {
		...
		it("throws InvalidPropertyException naming the offending field", () => {
			expect(() => new Bean({ name: "", slug: "ok" })).toThrow(
				InvalidPropertyException,
			);

			try {
				new Bean({ name: "", slug: "ok" });
				expect.unreachable("should have thrown");
			} catch (error) {
				expect(error).toBeInstanceOf(InvalidPropertyException);
				expect((error as InvalidPropertyException).property).toBe("name");
		...
		it("refuses a half-given identity at runtime", () => {
			...
			for (const { missing, context } of cases) {
				try {
					...
					expect(error).toBeInstanceOf(InvalidPropertyException);
					expect((error as InvalidPropertyException).property).toBe(missing);
				}
			}
		});
	...
	describe("schema", () => {
		...
		it("does not depend on the value-objects' defaults being valid", () => {
			...
			class BrokenEntity extends BetterEntity<typeof brokenProperties> {
	...
	describe("set", () => {
		...
		it("leaves the entity untouched when the new value is invalid", () => {
			...
			expect(() => bean.set("name", "")).toThrow(InvalidPropertyException);
			expect(bean.get("name")).toBe("alan");
			expect(bean.get("updatedAt")).toBeUndefined();
		});
		...
		it("leaves the post untouched when the nested value is invalid", () => {
			...
			expect(() => post.set("author", { name: "" })).toThrow(
				InvalidPropertyException,
			);
	...
	describe("accessors", () => {
		...
		it("refuses a blueprint key that collides with an existing member", () => {
			const collidingProperties = { schema: DefinedStringVO };

			class Colliding extends BetterEntity<typeof collidingProperties> {
			...
			try {
				...
				expect(error).toBeInstanceOf(OperationFailedException);
				expect((error as OperationFailedException).message).toContain("schema");
			}
	...
	describe("subclass construction", () => {
		...
		it("runs the subclass constructor body on every path", () => {
			...
			const countedProperties = { label: DefinedStringVO };

			class Counted extends BetterEntity<typeof countedProperties> {
	...
	describe("blueprint cycles", () => {
		it("rejects a self-referencing blueprint instead of overflowing", () => {
			...
			class Node extends BetterEntity<typeof nodeProperties> {
			...
			try {
				...
				expect(error).toBeInstanceOf(OperationFailedException);
				expect((error as OperationFailedException).message).toContain(
		...
		it("rejects the cycle when deriving the schema too", () => {
			class Loop extends BetterEntity<typeof loopProperties> {
		...
		it("does not mistake two properties of the same class for a cycle", () => {
			...
			class Review extends BetterEntity<typeof reviewProperties> {
	...
	describe("accessors on a subclass of a subclass", () => {
		...
		it("installs accessors for keys the parent's blueprint did not have", () => {
			const wideProperties = { ...beanProperties, extra: DefinedStringVO };
	...
	describe("fromJSON", () => {
		...
		it("rejects a payload with a value the schema refuses", () => {
			...
			expect(() => Bean.fromJSON({ ...bean.toJSON(), name: "" })).toThrow(
				InvalidDomainDataException,
			);
		});
		...
		it("rejects an aggregate whose nested payload is invalid", () => {
			...
			expect(() =>
				Post.fromJSON({
					...post.toJSON(),
					author: { ...post.get("author").toJSON(), name: "" },
				}),
			).toThrow(InvalidDomainDataException);
		});
