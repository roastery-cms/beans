 ...
 *   with an existing member of the class (`schema`, `toJSON`, `get`, `set`,
 *   `id`, `execute`, …).
 */
	...
	for (const key of missing)
		if (key in prototype)
			throw new PropertyNameCollisionException(
				key,
				source,
				`${label}: the blueprint property "${key}" collides with an existing member of the ${label.toLowerCase()} and cannot become an accessor. Rename it.`,
			);
