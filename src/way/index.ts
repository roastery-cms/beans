 ...
 * `defineEventHandler`, `eventedRegistry`) over the "precise" class forms
 * that sit right next to them in their own pillars. Reach past this barrel,
 ...
 *   its `handle` function).
 * - **Orchestration** — {@link eventedRegistry} (register use cases behind
 *   their dependencies, auto-publishing and auto-reacting to whatever they
 *   raise) and {@link IEventEmitter} (the contract to adapt an event bus
 *   to, so `eventedRegistry` can publish through it).
