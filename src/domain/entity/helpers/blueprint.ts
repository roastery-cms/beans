 ...
 * @see `PropertyRule` in `@roastery/beans/domain/entity/types` — what a rule may be.
...
export function blueprint<const PropertiesShape extends PropertiesShapeBase>(
	...
	return {
		...
		/** @inheritDoc */
			...
			// The shape itself, not a copy: with no rules there is nothing to
			// attach, and the blueprint object's identity is what memoizes the
			// derived schema and the compiled validator.
			return properties;
		},
	};
}
