 ...
 *
 * @see `blueprint` in `@roastery/beans/domain/entity/helpers` — returns this.
 */
export type BlueprintBuilder<PropertiesShape extends PropertiesShapeBase> = {
	...
	/**
	 * Closes the blueprint with **no** rules, returning the shape as it was
	 * given.
	 *
	 * Purely a spelling: a rule-less blueprint has always been expressible as a
	 * plain object literal (`const xProperties = { field: FieldVO }`), and
	 * `blueprint(shape).done()` produces exactly that same object — no `Rules`
	 * slot is attached, because `rulesOf` already reads a missing slot as an
	 * empty map. What it buys is consistency: every blueprint in a codebase can
	 * open with `blueprint({ … })` and close with either `.with({ … })` or
	 * `.done()`, instead of some starting with the helper and others with a
	 * bare literal.
	 *
	 * It exists because the alternative was `.with({})` — an empty object whose
	 * only job was to satisfy the builder. That reads like a rule map that
	 * someone forgot to fill in.
	 *
	 * @returns The blueprint, unchanged and rule-free.
	 *
	 * @example
	 * ```ts
