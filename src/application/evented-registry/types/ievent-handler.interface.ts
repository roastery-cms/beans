 ...
 * called once per matching event with the dependencies `eventedRegistry`
 ...
 * ```
 *
 * @see `IEventedRegistry.on` — where a concrete implementation is registered.
 */
export interface IEventHandler<Event extends DomainEvent, Deps> {
	/**
	 * Runs the reaction. `await`ed by `eventedRegistry` before the command
	 ...
	 * `EventedRegistryOptions.onError`.
	 *
