...
/**
 * The minimal, structural contract `commands` publishes every raised
 ...
 * listener) — a registry's own reactions are never dispatched through this
 * interface. They are registered and run through `.on()` instead, against
 * the registry's own internal map, precisely
...
export interface IEventEmitter {
	 ...
	 * @returns Nothing, or a `Promise` `commands` `await`s before
