 ...
 * `commands`'s existing per-reaction isolation (the optional
 ...
 * written by hand: nothing in `commands` itself needed to change.
 ...
 * `commands` itself — so it is directly usable with
 * `IEventedCommands.on(eventClass, GeneratedHandlerClass)`, gated by the
 ...
 * `commands` is keyed off the *event* class's name (`nameOf`),
 ...
 *   checking that what `commands` actually has available
 ...
 *   (`{ sendReceipt: typeof SendReceiptCommand }`). `commands` already
 ...
 *   `IEventedCommands.on(eventClass, ...)`.
 *
 * @see {@link EventHandlerClassOf} — the returned shape.
 * @see {@link IEventHandler} — the contract the generated class implements.
 * @see `IEventedCommands.on` in `./types/ievented-commands.interface` — where the generated class is registered.
