...
- **That order holds at every depth, including for a command another command dispatched.** A nested command's events are held until the boundary above it closes, then published in the order they were raised — the nested ones first. It has to work that way: a nested command inherits the boundary and so has none of its own to wait for, and publishing when *it* resolved would put the event inside a transaction that is still open.
...
- **A failing `emitter` does not fail the command.** By the time `emit` runs the command has resolved and, here, committed — so a throwing bus is isolated through `onError` exactly as a throwing reaction is, and the `CommandResult` still resolves. Reporting failure for an operation that succeeded would invite a retry that applies it twice. The event is lost instead, which is the two-step limit an outbox closes; see [Known limits](#known-limits).
...
- **One transaction at a time.** A `run` opened while another is still live throws `TransactionFailedException`, because this models a single connection and a single connection cannot hold two — Postgres answers `there is already a transaction in progress`. Joining instead would be worse than refusing: the snapshots cover the same stores, so a rollback in either would restore over rows the other had already committed. Await two transactions in sequence. Nothing legitimate hits this — `commands` opens a boundary only at the outermost marked command, and publication is deferred until every boundary closed, so not even a reaction can start one while another is alive.
...
- **Reads inside the transaction see its own writes**, and nothing models isolation between concurrent connections — the same statement as the rule above, seen from the read side. Fourth documented infidelity of the pillar, alongside UTF-16 string ordering and base64-ordered binary keys.
...
- **Commit and publish are still two steps.** Even inside a boundary, the events go out *after* the commit rather than as part of it, so a process that dies in between loses them. A bus that throws loses them the same way, since a failing `emit` is isolated through `onError` rather than propagated — the command already committed, and rejecting it would invite a retry that applies everything twice. The fix for both is an outbox — writing the events as rows in the same transaction and relaying them separately — which fits in a `CommandResult` without changing anything in `beans`, and is deliberately out of scope here.

- **The in-memory transaction runner does one transaction at a time.** Two overlapping `run` calls are refused with `TransactionFailedException` rather than interleaved, because the snapshots cover the same stores and a rollback in either would undo what the other committed. Await them in sequence. A real adapter has no such restriction; this one models a single connection.
