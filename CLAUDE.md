...
The root `@roastery/beans` only re-exports the two pillar classes — anything else lives behind a subpath. There is **no** `@roastery/beans/collections` barrel — the real subpaths are `/collections/schemas`, `/collections/value-objects`, `/collections/value-objects/custom`, `/collections/value-objects/custom/types`, `/collections/value-objects/optional` and `/collections/value-objects/nullable`. Inside the package itself, use the `@/*` path alias (mapped to `./src/*` in `tsconfig.json`).
